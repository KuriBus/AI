...
def purify_text_with_hyperclova_api(text: str) -> str:
    ...
    try:
        ...
        # 욕설 순화 전용 프롬프트 (설명 없이 결과만 출력)
        purify_prompt = f"""당신은 텍스트 순화 전문가입니다. 아래 텍스트에서 욕설, 비속어, 부적절한 표현만 정중한 표현으로 바꾸어 출력하세요.

**절대 규칙:**
- 설명, 해설, 부연설명 금지
- "상대방에게 모욕감을...", "이러한 표현 대신..." 같은 부가 설명 절대 금지
- 오직 순화된 텍스트만 출력
- 원문의 감정과 의미는 유지하되 욕설/비속어만 교체
